...
def map_world() -> Timeline:
    ...
    for i in range(2006, 2018):    
        c = (
            Map()
            .add(
                "中小学女生与男生的入学比例（%）", list(zip(list(df3['CountryName']),list(df3["{}".format(i)]))), "world",is_map_symbol_show = False)
...
def map_world1() -> Timeline:
    ...
    for i in range(2015, 2019):  
        c = (
            Map()
            .add(
            "女性参会人数占比%", list(zip(list(df2['CountryName']),list(df2["{}".format(i)]))), "world",is_map_symbol_show = False)
